...
type TbItemCatService interface {
	GetList(parentId int64) []*pojo.EasyUITreeNode
}

type tbItemCatService struct {
	engine *xorm.Engine
}

func NewTbItemCatService() TbItemCatService {
	return &tbItemCatService{
		engine: db.X,
	}
}

func (d *tbItemCatService) GetList(parentId int64) []*pojo.EasyUITreeNode {
	datalist := make([]models.TbItemCat, 0)
